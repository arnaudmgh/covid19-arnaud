...
***April 13th 2020***
Re-added US states; 10 most affected in terms of cases. 
...
(10 most affected states, ranked by recent cases)
```{r}
qplot(date, cases, colour = state, alpha = I(0.4), data=filter(usstates, state %in% staterank[1:10, "state"][[1]], date>"2020-03-10")) %>% ggplotly()
```

#### Number of Death per State
(Same 10 most affected states, ranked by recent cases (not deaths))

```{r}
qplot(date, deaths, colour = state, alpha = I(0.8), geom = "line", data=filter(usstates, state %in% staterank[1:10, "state"][[1]], date>"2020-03-10")) %>% ggplotly()
```
