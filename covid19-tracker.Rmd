...
totm <- mutate(totm, n_cases = conf_cases - recovered - n_deaths)
...
### Hubei, China
This is the province where the pandemic started. The dashed lines represent the confimed cases and the recovered cases.

```{r echo=FALSE}
qplot(date, n_cases, 
      geom = "line",
      data=filter(totm, grepl("Hubei", `Province/State`))) +
  geom_line(aes(date, conf_cases), lty = 2) +
  geom_line(aes(date, recovered), lty = 2)
```
